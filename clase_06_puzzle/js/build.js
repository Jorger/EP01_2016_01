...
var presionaPieza = function presionaPieza(fila, columna) {
    if (matrizDesorganiza[fila][columna].ocupado) {
        ...
        var esVacio = { vacio: false, fila: 0, columna: 0 };
        for (var i in direcciones) {
            ...
            if (enPosicion) {
                ...
                if (!matrizDesorganiza[posBusca.fila][posBusca.columna].ocupado) {
                    esVacio.vacio = true;
                    esVacio.fila = posBusca.fila;
                    esVacio.columna = posBusca.columna;
                    break;
                }
            }
        }
        if (esVacio.vacio) {
            //Indicar que el elemento que s emoverá...
            matrizDesorganiza[fila][columna].ocupado = false;

            var _estiloCelda2 = estiloCelda({ ocupado: false });

            var style = _estiloCelda2.style;
            var clase = _estiloCelda2.clase;

            _utils2.default.accesoDOM(fila + "_" + columna).setAttribute("style", style);
            _utils2.default.accesoDOM(fila + "_" + columna).classList.remove("fondo");
            _utils2.default.accesoDOM(fila + "_" + columna).innerHTML = "&nbsp;";
            //Ahora para el nuevo...
            matrizDesorganiza[esVacio.fila][esVacio.columna].ocupado = true;
            matrizDesorganiza[esVacio.fila][esVacio.columna].fila = matrizDesorganiza[fila][columna].fila;
            matrizDesorganiza[esVacio.fila][esVacio.columna].columna = matrizDesorganiza[fila][columna].columna;
            matrizDesorganiza[esVacio.fila][esVacio.columna].cont = matrizDesorganiza[fila][columna].cont;
            var estilo = estiloCelda({
                fila: matrizDesorganiza[fila][columna].fila,
                columna: matrizDesorganiza[fila][columna].columna,
                ocupado: true
            }).style;
            var idElment = esVacio.fila + "_" + esVacio.columna;
            _utils2.default.accesoDOM(idElment).setAttribute("style", estilo);
            _utils2.default.accesoDOM(idElment).classList.add("fondo");
            _utils2.default.accesoDOM(idElment).innerHTML = matrizDesorganiza[esVacio.fila][esVacio.columna].cont;
            //Validar si se ha completado...
        }
    }
    //Saber el valor...
    //console.log(matrizDesorganiza[fila][columna]);
    //console.log(matrizPuzzle[fila][columna]);
};
